...
public class TesseractWrapper
{
    ...
    private const float MinimumConfidence = 90;
    ...
    public object Recognize(Texture2D texture, Boolean CustomReturn = false)
    {
        if (_tessHandle.Equals(IntPtr.Zero))
            return null;
        ...
        {
            //Traitement
            IDictionary<string, object> dataTableWords  = new Dictionary<string, object>();
            dataTableWords.Add("words", words);
            dataTableWords.Add("confidence", confidence);
            dataTableWords.Add("MinimumConfidence", MinimumConfidence);
            return dataTableWords;
        }
        else
        {
